...
// IANET: 261122
void fillMotionParams(uint32 uirTargetTq, uint32 uirMaxTq, uint32 uirTgtPosn, uint32 uirMaxMotorSpeed, uint32 uirTgtVelocity)
{
    union
    {
        uint32 hl;
    ...
    } uiLclTargetTq, uiLclMaxTq, uiLclTgtPosn, uiLclMaxMotorSpeed, uiLclTgtVelocity;

    uiLclTargetTq.hl = uirTargetTq;
    uiLclMaxTq.hl = uirMaxTq;
    uiLclTgtPosn.hl = uirTgtPosn;
    uiLclMaxMotorSpeed.hl = uirMaxMotorSpeed;
    uiLclTgtVelocity.hl = uirTgtVelocity;

    *(ec_slave[0].outputs + (OUTPUT_OFFSET_TARGET_TQ)) = uiLclTargetTq.split.hh;
    *(ec_slave[0].outputs + (OUTPUT_OFFSET_TARGET_TQ + 1)) = uiLclTargetTq.split.hl;
    *(ec_slave[0].outputs + (OUTPUT_OFFSET_TARGET_TQ + 2)) = uiLclTargetTq.split.lh;
    *(ec_slave[0].outputs + (OUTPUT_OFFSET_TARGET_TQ + 3)) = uiLclTargetTq.split.ll;

    *(ec_slave[0].outputs + (OUTPUT_OFFSET_MAX_TQ)) = uiLclMaxTq.split.hh;
    *(ec_slave[0].outputs + (OUTPUT_OFFSET_MAX_TQ + 1)) = uiLclMaxTq.split.hl;
    *(ec_slave[0].outputs + (OUTPUT_OFFSET_MAX_TQ + 2)) = uiLclMaxTq.split.lh;
    *(ec_slave[0].outputs + (OUTPUT_OFFSET_MAX_TQ + 3)) = uiLclMaxTq.split.ll;

    *(ec_slave[0].outputs + (OUTPUT_OFFSET_MAX_MOTOR_SPEED)) = uiLclMaxMotorSpeed.split.hh;
    *(ec_slave[0].outputs + (OUTPUT_OFFSET_MAX_MOTOR_SPEED + 1)) = uiLclMaxMotorSpeed.split.hl;
    *(ec_slave[0].outputs + (OUTPUT_OFFSET_MAX_MOTOR_SPEED + 2)) = uiLclMaxMotorSpeed.split.lh;
    *(ec_slave[0].outputs + (OUTPUT_OFFSET_MAX_MOTOR_SPEED + 3)) = uiLclMaxMotorSpeed.split.ll;

    *(ec_slave[0].outputs + (OUTPUT_OFFSET_TARGET_POSN)) = uiLclTgtPosn.split.hh;
    *(ec_slave[0].outputs + (OUTPUT_OFFSET_TARGET_POSN + 1)) = uiLclTgtPosn.split.hl;
    *(ec_slave[0].outputs + (OUTPUT_OFFSET_TARGET_POSN + 2)) = uiLclTgtPosn.split.lh;
    *(ec_slave[0].outputs + (OUTPUT_OFFSET_TARGET_POSN + 3)) = uiLclTgtPosn.split.ll;
}
            ...
            else
            {
                ...
                while (1)
                {
                    ...
                    if (delMeOpnExpectedFlag == 1)
                    {
                        ...
                        printf("\n Operate Motor");
                        fillMotionParams(iPosActualValue.hl + 90000, uint16 uirTqFeedFwd, 10);
                    }
                }
            }
